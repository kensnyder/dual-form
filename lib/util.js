...
var util = {
	...
	diveSet: function(object, path, setTo) {
		...
		while (path.length > 1) {
			attr = path.shift();
			if (path[0] == '[]') {
				if (!Array.isArray(workingObject[attr])) {
					workingObject[attr] = [];
				}
			}
			else {
				if (typeof workingObject[attr] != 'object') {
					workingObject[attr] = {};
				}
			}
		...
		else if (Array.isArray(workingObject[attr])) {
			workingObject[attr].push(setTo);
		}
		else if (!!workingObject[attr]) {
			workingObject[attr] = [ workingObject[attr], setTo ];
		}
		else {
