...
elements.FormElement = Element.subclass({
	...
	addValidator: function(validator) {
		...
		if (typeof validator == 'function') {
			...
			fn = validators[args.shift()];
			this.validators.push( fn.apply(this, args) ) ;
		}
		else {
			throw new TypeError('[dual-form] Validator must be a function or a key in Form.validators.');
		}
		return this;
	}
});
...
elements.Button = elements.FormElement.subclass({
	tagName: 'button'
});
...
elements.Range = elements.Input.subclass({
	construct: function() {
		this.attributes.type = 'range';
	}
});
elements.Submit = elements.Input.subclass({
	construct: function() {
		this.attributes.type = 'submit';
	}
});
elements.Reset = elements.Input.subclass({
	construct: function() {
...
elements.Checkbox = elements.Input.subclass({
	decorator: Form.decorators.checkboxBr,
	construct: function() {
